...
//start script
			async function starts() {
				const Rpan = new WAConnection()
				Rpan.version = [2, 2143, 3]  //jika ada update dari WhatsApp web silahkan ubah
				Rpan.logger.level = 'warn'
				console.log(color(figlet.textSync('Rpan-Bot⚡', {
					font: 'Standard',
			...
			fs.existsSync(`./settings/${session_name}`) && Rpan.loadAuthInfo(`./settings/${session_name}`)
			Rpan.on('connecting', () => {
			...
			await Rpan.connect({timeoutMs: 30*1000})
			fs.writeFileSync(`./settings/${session_name}`, JSON.stringify(Rpan.base64EncodedAuthInfo(), null, '\t'))

					
//greetings
				Rpan.on("group-participants-update", async (anu) => {
    try {
      groupMet = await Rpan.groupMetadata(anu.jid);
      ...
      console.log(anu);
      try {
        pp_user = await Rpan.getProfilePicture(mem);
      } catch (e) {
        pp_user =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      try {
        pp_grup = await Rpan.getProfilePicture(anu.jid);
      } catch (e) {
        pp_grup =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      if (anu.action == "add" && mem.includes(Rpan.user.jid)) {
        Rpan.sendMessage(anu.jid, "Minasankon'nichiwa ", "conversation");
      }
hehe = await getBuffer(pp_user)
if (anu.action == 'add' && !mem.includes(Rpan.user.jid)) {
             const mdata = await Rpan.groupMetadata(anu.jid)
             
             const memeg = mdata.participants.length
             const thu = await Rpan.getStatus(anu.participants[0], MessageType.text)
             const num = anu.participants[0]
             const bosco1 = await Rpan.prepareMessage("0@s.whatsapp.net", hehe, MessageType.location,{ thumbnail: hehe})
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                let v = Rpan.contacts[num] || { notify: num.replace(/@.+/, '') }
                ...
                Rpan.sendMessage(mdata.id, welcomeButt, MessageType.buttonsMessage, { caption: 'hehe', "contextInfo": { "mentionedJid" : [num], },})
                 }
            if (anu.action == 'remove' && !mem.includes(Rpan.user.jid)) {
                const mdata = await Rpan.groupMetadata(anu.jid)
                const num = anu.participants[0]
                const bosco3 = await Rpan.prepareMessage("0@s.whatsapp.net", hehe, MessageType.location,{ thumbnail: hehe})
			    const bosco4 = bosco3.message["ephemeralMessage"] ? bosco3.message.ephemeralMessage : bosco3
                let w = Rpan.contacts[num] || { notify: num.replace(/@.+/, '') }
                ...
                Rpan.sendMessage(mdata.id, goodbyeButt, MessageType.buttonsMessage, { caption: 'hehe', "contextInfo": { "mentionedJid" : [num], },})
                  }
				} catch (e) {
				...
				Rpan.on('chat-update', async (mek) => {
					try {
						...
						const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = Rpan.user.phone
						const botNumber = Rpan.user.jid
						const ownerNumber = [`${ownernumber}@s.whatsapp.net`] 
						const isGroup = from.endsWith('@g.us')
						const totalchat = await Rpan.chats.all()
						const sender = mek.key.fromMe ? Rpan.user.jid : isGroup ? mek.participant : mek.key.remoteJid
						const isOwner = mek.key.fromMe ? Rpan.user.jid : ownerNumber.includes(sender)
						const conts = mek.key.fromMe ? Rpan.user.jid : Rpan.contacts[sender] || { notify: jid.replace(/@.+/, '') }
						const pushname = mek.key.fromMe ? Rpan.user.name : conts.notify || conts.vname || conts.name || '-'
						
						//apaya
						const isAntiLink = isGroup ? antilink.includes(from) : false
						const isAntiVirtex = isGroup ? antivirtex.includes(from) : false
						const groupMetadata = isGroup ? await Rpan.groupMetadata(from) : ''
    ...
    return new Promise((resolve, reject) => {
        ...
        resolve(`${(bytes / (1024 ** i)).toFixed(1)} ${sizes[i]}`);
			...
			const checkLimit = (sender) => {
				...
				for (let lmt of _limit) {
					if (lmt.id === sender) {
						...
						if (limitCounts <= 0) return Rpan.sendMessage(from,`Limit kamu sudah habis`, text,{ quoted: mek})
						Rpan.sendMessage(from, lang.limitcount(isPremium, limitCounts), text, { quoted : mek})
						found = true
					}
				}
					if (found === false) {
			...
			const isLimit = (sender) =>{ 
				...
				for (let i of _limit) {
					if (i.id === sender) {
						...
						if (limits >= limitawal ) {
							position = true
							Rpan.sendMessage(from, lang.limitend(pushname), text, {quoted: mek})
							return true
						} else {
							_limit
							position = true
			...
			//function
			const reply = (teks) => {
				Rpan.sendMessage(from, teks, text, { quoted: mek, thumbnail: thumbnail})
			}
			const isUrl = (url) => {
			    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? Rpan.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : Rpan.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
			}
		    const sleep = async (ms) => {
				return new Promise(resolve => setTimeout(resolve, ms));
			}
			const sendMess = (hehe, teks) => {
				Rpan.sendMessage(hehe, teks, text)
			}
			...
			const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './sticker' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        Rpan.sendMessage(to, media, MessageType.sticker,{quoted:mek})
							...
							download(url, filename, async function () {
								...
								if(mime === "image/gif"){
									...
									if(mime.split("/")[0] === "audio"){
										...
										Rpan.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
			...
			async function sendFileFromUrl(from, url, caption, mek, men) {
				...
				if(mime.split("/")[0] === "audio"){
					...
					return Rpan.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, thumbnail: Buffer.alloc(0), mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
				}
...
for (e of b){
let ingfo = await Rpan.groupMetadata(e)
grup.push(ingfo)
}
return grup
}  
...
// send message button
				const sendButMessage = (id, text1, desc1, but = [], options = {}) => {
					const buttonMessage = {
						...
						Rpan.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options);
					};
				const sendButLocation = async (id, text1, desc1, gam1, but = [], options = {}) => {
					them = gam1
					mediaxxaa = await Rpan.prepareMessage(id, them, MessageType.location, {thumbnail: them})
					...
					const buttonMessages = {
						...
						Rpan.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
						}
				const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await Rpan.prepareMessage(id, them, MessageType.video)
				...
				const sendButImage = async(id, text1, desc1, vid1, but = [], options = {}) => {
					them = vid1
					mediaxxaa = await Rpan.prepareMessage(id, them, MessageType.image, {thumbnail: Buffer.alloc(0)})
		        ...
		        await Rpan.sendMessage(from, `Hmm maap nih gua kick, dilarang share link di group ini`, text , {quoted: mek})
		        Rpan.groupRemove(from, [kic]).catch((e)=>{reply(`Bot Harus Jadi Admin`)})
		        }
...
Rpan.updatePresence(from, Presence.composing)
...
Rpan.sendMessage(from, teks, text, {quoted:mek, contextInfo:{mentionedJid: [sender]}})
setTimeout( () => {
Rpan.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
}, 1000)
setTimeout( () => {
Rpan.updatePresence(from, Presence.composing)
reply("SELAMAT JALAN")
			...
			const isRpan = checkRegisteredUser(sender)
  ...
  download(url, filename, async function () {
	...
	Rpan.sendMessage(from, hasil, type, options).catch(e => {
	fetch(link).then((hasil) => {
	Rpan.sendMessage(from, hasil, type, options).catch(e => {
	Rpan.sendMessage(from, { url : link }, type, options).catch(e => {
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
Rpan.sendMessage(from, teks, text, {quoted: fgif})
break
case 'get':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'owner':{
	if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			const vname = Rpan.contacts[i] != undefined ? Rpan.contacts[i].vname || Rpan.contacts[i].notify : undefined
			ini_list.push({
				"displayName": `${ownername}`,
				"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
				})
				}
				hehe = await Rpan.sendMessage(from, {
					"displayName": `${ini_list.length} kontak`,
					"contacts": ini_list 
					}, 'contactsArrayMessage', { quoted: mek })
					Rpan.sendMessage(from, `Nih Kak Contact Owner Ku, Cuma Sv Nomor Cewe Ya 🤝`, text, {quoted: hehe})
				}
			break
case 'donasi':
var aprian = `
...
*MAKASIH*
`
Rpan.sendMessage(from, aprian, text,{quoted: fgif})
break
case 'rules':
...
Jawaban = '\n*'+randKey.result.desc +'*'
setTimeout( () => {
Rpan.sendMessage(from, Jawaban, text, {quoted: mek})
}, 30000)
setTimeout( () => {
Rpan.sendMessage(from, '_10 Detik lagi�_', text) // ur cods
}, 20000) // 1000 = 1s,
setTimeout( () => {
Rpan.sendMessage(from, '_20 Detik lagi_�', text) // ur cods
}, 10000) // 1000 = 1s,
setTimeout( () => {
Rpan.sendMessage(from, '_30 Detik lagi_�', text) // ur cods
}, 2500) // 1000 = 1s
Rpan.sendMessage(from, Pertanyaan, text, {quoted: mek})
break
case 'family100':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
setTimeout( () => {
Rpan.sendMessage(from, '*'+ Pertanyaan +'*', text, {quoted: mek }) // ur cods
}, 0) // 1000 = 1s,
break         
                case 'kisahnabi':
                if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'waifu':
    ...
    if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    	  ...
    	  const bb = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)

            const buttonnsss = [{buttonId:`.${command}`,buttonText: {displayText: 'Next'}, type: 1}]

            const ButtonssMessages = {

            contentText: `_� By Revan_`,
            ...
            await Rpan.sendMessage(from, ButtonssMessages, MessageType.buttonsMessage, {quoted: fgif})
...
Rpan.sendMessage(from, soun, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound1':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
satu = fs.readFileSync('./mp3/sound1.mp3');
Rpan.sendMessage(from, satu, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound2':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
dua = fs.readFileSync('./mp3/sound2.m4a');
Rpan.sendMessage(from, dua, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound3':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
tiga = fs.readFileSync('./mp3/sound3.m4a');
Rpan.sendMessage(from, tiga, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound4':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
empat = fs.readFileSync('./mp3/sound4.m4a');
Rpan.sendMessage(from, empat, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound5':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
lima = fs.readFileSync('./mp3/sound5.m4a');
Rpan.sendMessage(from, lima, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound6':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
enam = fs.readFileSync('./mp3/sound6.m4a');
Rpan.sendMessage(from, enam, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound7':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
tujuh = fs.readFileSync('./mp3/sound7.m4a');
Rpan.sendMessage(from, tujuh, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break									
case 'sound8':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
delapan = fs.readFileSync('./mp3/sound8.m4a');
Rpan.sendMessage(from, delapan, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound9':
sembilan = fs.readFileSync('./mp3/sound9.m4a');
Rpan.sendMessage(from, sembilan, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound10':
sepuluh = fs.readFileSync('./mp3/sound10.m4a');
Rpan.sendMessage(from, sepuluh, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound11':
sebelas = fs.readFileSync('./mp3/sound11.m4a');
Rpan.sendMessage(from, sebelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound12':
duabelas = fs.readFileSync('./mp3/sound12.m4a');
Rpan.sendMessage(from, duabelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'toptt':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
if (!isQuotedAudio && !isQuotedVideo) return reply('Reply vnnya atau videonya!')
encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
media =await Rpan.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('Gagal mengkonversi audio ke ptt')
topt = fs.readFileSync(ran)
Rpan.sendMessage(from, topt, audio, {mimetype: 'audio/mp4', quoted: fgif, ptt:true})
})
break
case 'tomp3':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
Rpan.sendMessage(from, bufferlkj, audio, {mimetype: 'audio/mp4', quoted: fgif})
fs.unlinkSync(ran)
})
break
case 'tovideo':
...
owgi =await Rpan.downloadAndSaveMediaMessage(ger)
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			const media = await Rpan.downloadAndSaveMediaMessage(encmedia)
			ran = getRandom('.webp')
			await ffmpeg(`./${media}`)
			.input(media)
			...
			.on('end', function () {
				console.log('Finish')
				Rpan.sendMessage(from, fs.readFileSync(ran), sticker, { quoted: mek })
				fs.unlinkSync(media)
				fs.unlinkSync(ran)
				})
...
Rpan.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek, caption: 'FOLLOW IG @revanabi_ ANJ'})
break
case 'tiktokwm':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
reply(lang.wait())
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					Rpan.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mp4',filename: `Tiktok Music.mp3`,quoted: mek})
					})
...
case 'pinterest':
            ...
            await Rpan.sendMessage(from,di,image,{quoted: mek})
            break
case 'play': case 'song':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'naruto': case 'kaneki': case 'shinomiya': case 'kurama': case 'rimuru': case 'akira': case 'dazai': case 'miku': case 'mikasa': case 'eren': case 'titan': case 'levi': case 'sakura': case 'hinata': case 'neji': 
            ...
            await Rpan.sendMessage(from,ao,image,{quoted: fgif, caption: `Tuh foto ${command}`})
            break
case 'anime':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    	  ...
    	  const ab = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, ButtsMessages, MessageType.buttonsMessage, {quoted : fgif})
...
case 'teknologi':
    	  ...
    	  const agb = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, ButsMessages, MessageType.buttonsMessage, {quoted : fgif})
    	  ...
    	  const aba = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, BttsMessages, MessageType.buttonsMessage, {quoted : fgif})
    	  ...
    	  const jb = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, BtsMessages, MessageType.buttonsMessage, {quoted : fgif})
...
const pbou = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
const bulonss = [{buttonId:`.${command}`,buttonText: {displayText: 'Next'}, type: 1}]
            ...
            await Rpan.sendMessage(from, Bsessages, MessageType.buttonsMessage, {quoted : fgif})
            fs.unlinkSync(`./${sender}.jpeg`)
            break
case 'otaku':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            Rpan.sendMessage(from,ram,image,{quoted:fgif,caption:rem})
            break
    case 'komiku':
    if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
*Chapter Akhir* : ${komik.chapter_akhir}`
    ...
    if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
            ...
            await Rpan.sendMessage(from,li,image,{quoted: fgif})
            break
                    case 'walpaper':
                if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    	  ...
    	  const pb = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, BsMessages, MessageType.buttonsMessage, {quoted : fgif})
...
case 'muslim':
    	  ...
    	  const pbo = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, BsotMessages, MessageType.buttonsMessage, {quoted : fgif})
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    	  ...
    	  const pubo = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, Bessages, MessageType.buttonsMessage, {quoted : fgif})
            fs.unlinkSync(`./${sender}.jpeg`)
            break
case 'igstalk':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		Rpan.sendMessage(from, siapaye, image,{quoted: fgif, caption: infonye})
		            break    
		case 'ghstalk': 
            case 'githubstalk': 
            case 'ghuser':
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    	  ...
    	  const po = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, BsoMessages, MessageType.buttonsMessage, {quoted : fgif})

            fs.unlinkSync(`./${sender}.jpeg`)
            break
//group
case 'daftar': case 'verify': case 'verif':
			if (isRpan) return  reply(lang.regis())
			try {
					ppregis = await Rpan.getProfilePicture(sender)
				} catch {
...
case 'truth':
                if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					const trut =['Pernah suka sama siapa aja? beRpan lama?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
					Rpan.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
case 'dare':
                if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					break
case 'sangecek':
					 // Fix Bug By Aprii				
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+sange+'*\n\nJawaban : '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycek':
					 // Fix Bug By Aprii				
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+gayy+'*\n\nJawaban : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicek':
					 // Fix Bug By Aprii				
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+lesbii+'*\n\nJawaban : '+ bi+'%', text, { quoted: mek })
					break
                case 'gantengcek':
					 // Fix Bug By Aprii				
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+ganteng+'*\n\nJawaban : '+ teng+'%', text, { quoted: mek })
					break
		case 'cantikcek':
					 // Fix Bug By Aprii				
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+cantik+'*\n\nJawaban : '+ tik+'%', text, { quoted: mek })
					break
...
case 'kapankah':
				...
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					//tes
					break
case 'jadian':
				if (!isGroup) return reply(lang.group())
		...
		case 'apakah':
				...
				if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
					...
					Rpan.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					//tes
					break
...
Rpan.sendMessage(from, tun, text,{quoted: fgif})
break
case 'faktaunik':
  if (!isGroup) return reply(lang.group())
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
Rpan.sendMessage(from, taa, text,{quoted: fgif})
break
case 'quotesanime':
  if (!isGroup) return reply(lang.group())
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
Rpan.sendMessage(from, a, text,{quoted: fgif})
break
...
case 'tourl':
        if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
            ...
            owgi = await Rpan.downloadMediaMessage(boij)
...
var has = kls.split("|")[0];
var kas = kls.split("|")[1];
Rpan.sendMessage(from, `${has}${kas}` , text, { quoted: mek })
break
        case 'futanari':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
               reply(lang.wait())
                buffer = await getBuffer(`https://api.dapuhy.xyz/api/nsfw/nsfwfutanari?apikey=B0ZTfKR00k`)
                const bba = await Rpan.prepareMessage(from, buffer, MessageType.image)
            ...
            await Rpan.sendMessage(from, ButonsMerssages, MessageType.buttonsMessage, {quoted : fgif})
                break 
                case 'neko':
                if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
                reply(lang.wait())
                buffer = await getBuffer(`https://api.dhamzxploit.my.id/api/neko`)
                const zd = await Rpan.prepareMessage(from, buffer, MessageType.image)
            ...
            await Rpan.sendMessage(from, BeutonMesages, MessageType.buttonsMessage, {quoted : fgif})
                break
    case 'anal':
          if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
          reply(lang.wait())
...
const vcard = 'BEGIN:VCARD\n'  
            ...
            + `FN: Revan\n`  
            + `Org: Revan;\n` 
            + `TEL;type=CELL;type=VOICE;waid=6281930239749:+6281930239749\n`  
            + 'END:VCARD'  
  Rpan.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
if (!isOwner) return reply(lang.owner(botname))
await Rpan.toggleDisappearingMessages(from)
reply("yahaha")
break
case 'ttp':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
break
                ...
                Rpan.sendMessage(from, ini_buffer, sticker, {quoted: fgif})
                break 
                case 'group':
					apri = 'PILIH BEMRO'
...
case "closegc": 
        ...
        Rpan.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break; 
      case "opengc":
        if (!mek.key.fromMe && !isGroupAdmins) return reply("Only admin");
        if (!isBotGroupAdmins) return reply("Bot not admin");
        ...
        Rpan.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break; 
case 'hentai':
case 'ahegao':
case 'ass':
...
let wowu = (await axios.get(`https://raw.githubusercontent.com/jepribarus/JB-Api/main/nsfw/${command}.json`)).data
    	  ...
    	  const botak = await Rpan.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), MessageType.image)
            ...
            await Rpan.sendMessage(from, ButtonsMessages, MessageType.buttonsMessage, {quoted : fgif})
...
const sa = await Rpan.prepareMessage(from, buffer, MessageType.image)

            const butons = [{buttonId:`.${command}`,buttonText: {displayText: 'Next'}, type: 1}]

            const ButonsMessages = {

            contentText: `_� By Revan`,
            ...
            await Rpan.sendMessage(from, ButonsMessages, MessageType.buttonsMessage, {quoted : fgif})
                break 
*/
                case 'nekoh':
                if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
               reply(lang.wait())
                ...
                const bc = await Rpan.prepareMessage(from, buffer, MessageType.image)
            ...
            await Rpan.sendMessage(from, ButtonMessages, MessageType.buttonsMessage, {quoted : fgif})
                break
        case 'add':
  try {
  if (!isGroup) return reply(lang.group())
  ...
  response = await Rpan.groupAdd(from, [entah])
...
case 'memegenerator': case 'memegen':{
									...
									try {
									   ...
									   var mediiia = await Rpan.downloadMediaMessage(enmedia)
										...
										Rpan.sendMessage(from, resu, image, {caption:'.stikerin bang', thumbnail: Buffer.alloc(0), quoted: mek})
						...
						if (args.length < 1) return reply(`Kirim perintah *.${command}* Alphabot`)
									...
									try {
										...
										var teks2 = args.join(' ')
										var enmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
										var mediia = await Rpan.downloadMediaMessage(enmedia)
...
case 'leave':
			...
			if (!isOwner) return reply(lang.owner(botname))
			setTimeout( () => {
			Rpan.groupLeave(from) 
			}, 2000)
			setTimeout( () => {
			Rpan.sendMessage(from, 'Sayonara👋', text)
			}, 0)
			break
case 'hidetag':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			var value = q
			var group = await Rpan.groupMetadata(from)
			...
			Rpan.sendMessage(from, options, text)
			break
case 'linkgrup':case 'linkgroup': case 'linkgc':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
			linkgc = await Rpan.groupInviteCode(from)
			yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
			Rpan.sendMessage(from, yeh, text, { quoted: mek })
			break  
case 'tagall':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			if (!isBotGroupAdmins) return reply(lang.adminB())
					await Rpan.groupUpdateSubject(from, `${q}`)
					Rpan.sendMessage(from, `Sukses Mengubah Nama Grup Menjadi ${q}`, text, { quoted: mek })
			break          
...
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			...
			if (!isGroupAdmins) return reply(lang.admin(groupName))
			if (!isBotGroupAdmins) return reply(lang.adminB())
					await Rpan.groupUpdateDescription(from, `${q}`)
					Rpan.sendMessage(from, `Sukses Mengubah Desk Grup Menjadi ${q}`, text, { quoted: mek })
			break   
case 'kick':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!isGroup) return reply(lang.group())
			...
			await Rpan.groupRemove(from, [kickya])
...
case 'setppbott':
				if (!isOwner) return reply('*Only Owner bot*')
					...
					media = await Rpan.downloadAndSaveMediaMessage(enmedia)
					await Rpan.updateProfilePicture(botNumber, media)
					reply('Makasih profil barunya')
					break
					case 'bc': 
					if (!isOwner) return reply(lang.owner(botname)) 
					if (args.length < 1) return reply('.......')
					anu = await Rpan.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buffer = await Rpan.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							Rpan.sendMessage(_.jid, buffer, image, {caption: `* PESAN BROADCAST *\n\n${body.slice(4)}`})
						}
...
case 'bc2': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await Rpan.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							Rpan.sendMessage(_.jid, bc, image, {quoted:mek,caption: `* PESAN SIARAN BOT *\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
Rpan.sendMessage(_.jid, 
			...
			"buttonText": {"displayText": "MENU BOT"
             ...
             break
				 ...
				 Rpan.chatRead (from)
					if (!isGroup) return reply(lang.group())
					if (!isOwner) return reply(lang.owner(botname))
					Rpan.blockUser (`${body.slice(7)}@c.us`, "add")
					Rpan.sendMessage(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(lang.group())
					if (!isOwner) return reply(lang.owner(botname))
				    Rpan.blockUser (`${body.slice(9)}@c.us`, "remove")
					Rpan.sendMessage(from, `Perintah Diterima, membuka ${body.slice(9)}@c.us`, text)
					break   				
case 'nightcore':{
					...
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						...
						Rpan.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek,duration:99999999999999999999999})
						fs.unlinkSync(ran)
					   })}
				  break      
case 'bass': {
...
case 'slowmo': case 'slow':{
								try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
											...
											Rpan.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
											fs.unlinkSync(ran)
											})
											} catch (e) {
												reply('Error!')
...
case 'ssweb':  
                   ...
                   Rpan.sendMessage(from, anu, image, {caption: `BERHASIL KAK`, quoted: fgif})
                   break
case 'infogempa':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
Rpan.sendMessage(from, buffer, image, {quoted: fgif, caption: aku})
break
case 'upswteks':
            if (!q) return reply('Isi teksnya!')
            Rpan.sendMessage('status@broadcast', `${q}`, extendedText)
            fakegroup(`Sukses Up story wea teks ${q}`)
            break
    case 'upswimage':
            if (isQuotedImage) {
            const swsw = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            cihcih = await Rpan.downloadMediaMessage(swsw)
            Rpan.sendMessage('status@broadcast', cihcih, image, { caption: `${q}` })
            bur = `Sukses Upload Story Image dengan Caption: ${q}`
            Rpan.sendMessage(from, bur, text, { quoted: mek })
            } else {
    ...
    case 'upswvideo':
            ...
            cihcih = await Rpan.downloadMediaMessage(swsw)
            Rpan.sendMessage('status@broadcast', cihcih, video, { caption: `${q}` }) 
...
case 'robot':{
									...
									exec(`ffmpeg -i ${media} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
										...
										if (err) return reply('Error!')
										hah = fs.readFileSync(ran)
										Rpan.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, quoted: mek})
										fs.unlinkSync(ran)
										})
										}
									break
...
case 'tupai':{
									try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
											...
											if (err) return reply('Error!')
											hah = fs.readFileSync(ran)
											Rpan.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek,duration: 999099})
											fs.unlinkSync(ran)
											})
											 } catch (e) {	
												reply(mess.error)
...
case 'toimg':{
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					mediaa = await Rpan.downloadAndSaveMediaMessage(encmediaa)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${mediaa} ${ran}`, (err) => {
					fs.unlinkSync(mediaa)
					if (err) return reply('Eror')
					buffer = fs.readFileSync(ran)
					Rpan.sendMessage(from, buffer, image, {quoted: mek, thumbnail:Buffer.alloc(0), caption: 'Done'})
					fs.unlinkSync(ran)
					})
					}
					break   
case 'nuliskiri':{
									...
									.on('exit', () => {
										Rpan.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'nuliskanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Rpan.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'foliokiri':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Rpan.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
						...
						case 'foliokanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										Rpan.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Hati-hati ketahuan!`})
...
case 'brainly':
					...
					Rpan.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
		            })              
break		
case 'searchgc':
if (!isRpan) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
try {
	if (!isOwner) return reply(lang.owner(botname))
return Rpan.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`RpanBot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
